% System of differential equations for the description of the motion of a
% single-body meteoroid in pancake phase in atmosphere with WGS84 Earth model in a
...
% y(7) = vector mass, kg
% y(8) = expansion radius, m (initial value is always 0)
%
...
% Compute effective cross section without expansion, m^2
S0=F*(y(7)/rho_M)^(2/3);

% Compute effective asteroid radius, m
R0=sqrt(S0/pi);

% Expansion radius, m
RL=R0+y(8);

% Compute effective cross section with expansion, m^2
SL=pi*RL*RL;

% Acceleration along x, m/s^2 (include Coriolis acceleration)
dydt(1)=-G*M*y(4)/(r^3)-(Ga*rho_a*Vr*SL*(y(1)-VX))/y(7)+(OM^2)*y(4)+2*OM*y(2);

% Acceleration along y, m/s^2 (include Coriolis acceleration)
dydt(2)=-G*M*y(5)/(r^3)-(Ga*rho_a*Vr*SL*(y(2)-VY))/y(7)+(OM^2)*y(5)-2*OM*y(1);

% Acceleration along z, m/s^2
dydt(3)=-G*M*y(6)/(r^3)-(Ga*rho_a*Vr*SL*(y(3)-VZ))/y(7);
...
% Mass ablation, kg/s
dydt(7)=-Ga*(CH/Q)*rho_a*SL*Vr*Vr*Vr;
...
% Lateral expansion speed, m/s
dydt(8)=Vr*sqrt(7*rho_a/(2*rho_M));

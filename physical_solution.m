...
% numerical integration. The solution is physically acceptable if Strength >= dynamic pressure and elevation >= final elevation.
%
% Input:
...
for i=1:length(y(:,4))
    ...
    % Speed sound at quota_meteoroide, m/s
    vs=20.0468*sqrt(T_a); 
    ...
    % Interpolate air density
    rho_a=interp1(quota_vento,density,quota_meteoroide);
    % Dynamic pressure, MPa
    Pdyn=Ga*rho_a*(y(i, 1)^2+y(i, 2)^2+y(i, 3)^2)/10^6; 
   
   if Strength - Pdyn >= 0 && quota_meteoroide >= Qfin
       N=N+1;
   end
end
